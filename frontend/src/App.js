	...
	const renderPlot = () => (
		<div className="fixed inset-0 bg-black/50 flex items-center justify-center p-8">
			<div className="bg-white rounded-2xl p-8 w-full max-w-4xl">
				...
				<div
					className={`${
						selectedAlgorithm === "5" ? "grid grid-cols-2 gap-6" : ""
					} h-96`}
				>
							...
							<div className="mt-4 space-y-2">
								{selectedAlgorithm == 2 && (
									<p className="text-xl text-gray-700">
										Restart: {cubeStates[currentFrame][3]}
									</p>
								)}
								
								<p className="text-xl text-gray-700">
									{selectedAlgorithm != 6 ?'Iteration':'Population'}: {cubeStates[currentFrame][0]}
								</p>
								<p className="text-xl text-gray-700">
								...
								{selectedAlgorithm == 6 && (
									<p className="text-xl text-gray-700">
										Maximum Score on Population {cubeStates[currentFrame][0]}: {cubeStates[currentFrame][3]}
									</p>
								)}
								{selectedAlgorithm == 6 && (
									<p className="text-xl text-gray-700">
										Average Score: {cubeStates[currentFrame][4]}
									</p>
								)}
								{selectedAlgorithm === "5" &&
									cubeStates[currentFrame][3] !== undefined && (
										<p className="text-xl text-gray-700">
	...
	return (
		<div className="min-h-screen bg-gradient-to-br from-indigo-100 via-purple-100 to-pink-100">
			{step === 1 ? (
				...
				<ResultGA maxIteration={maxIteration} population={population} />
			) : (
				<ResultPage
