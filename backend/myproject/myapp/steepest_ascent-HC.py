...
def main():
    ...
    # Konversi kubus ke format daftar 3D yang bisa diubah ke JSON
    final_cube_array = cube.cube.tolist()  # Konversi array NumPy ke daftar

    # Buat dictionary hasil akhir untuk diserialisasi ke JSON
    result = {
        "final_score": final_score,
        "final_cube": final_cube_array
    }

    # Kirim POST request ke endpoint Django
    response = requests.post("http://127.0.0.1:8000/api/receive-cube/", json=result)

    # Tampilkan respons dari Django
    print(response.json())
